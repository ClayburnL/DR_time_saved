...
function [protocol, protocol_original_time_sum, protocol_time_saving] = Read_xml_fn_v5(XMLfile)
...
%     protocol_time_saving=cell(1,1);
%     protocol_original_time_sum=cell(1,1);
%     protocol_original_time_sum{1,1}=0;
    protocol_original_time_sum=0;
    
    if exist('DR_seq_name', 'var')
        for i=1:length(seq_name)
        ...
        %% Write to .xls
        Append2xls_v4(scanner, protocol, seq_name, seq_time, DR_seq, original_seq, DR_seq_time, original_seq_time, time_saving, time_saving_percent, protocol_time_saving, protocol_time_saving_percent);
    else
        for i=1:length(seq_name)
